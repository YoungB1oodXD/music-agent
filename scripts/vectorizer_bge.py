...
class ChromaDBBuilder:
    ...
    def add_vectors(
        self, df: pd.DataFrame, vectorizer: BGEVectorizer, start_idx: int = 0
    ):
        """批量添加向量到数据库"""
        if self.collection is None:
        ...
        for i in tqdm(
            range(start_idx, total_rows, vectorizer.batch_size),
            total=total_batches,
            desc="向量化",
        ):
                ...
                metadatas.append(
                    {
                        ...
                        "genres_all": str(row.get("genres_all", "")),
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=BATCH_SIZE,
        help=f"批次大小（默认 {BATCH_SIZE}，增大可提速但吃内存）",
    )
    ...
    start_idx = (
        ...
        else (existing_count if resume else 0)
    )
    total_rows = len(df)
    ...
    if (
        resume
        and start_idx_override is not None
        and start_idx_override < existing_count
    ):
        logger.warning(
